...
class SchedulePDFView(PrintableMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request, unit_id, date_from):
        ...
        context_data={'orders1':SchedulePDFView.global_orders,'date_from':self.date_from}
        print('Finished')
        ...
        # response = self.render_printable(False, self.template_name, context_data, header_template="", pdf_options={"orientation": "landscape"})
    ...
    def get_rows(self, table=None):
        ...
        rows = [
            [
                ...
                order.ordertransport.weigh_bridge_date if OrderTransport.objects.filter(order_id= order.id).exists() else None,
                order.ordertransport.weigh_bridge_comments if OrderTransport.objects.filter(order_id= order.id).exists() else None,
